...
const WordCard = ({ text = "N/A", color = "#000000", partOfSentence }) => {
    ...
    const dispatchType = partOfSentence
    const dispatch = useDispatch()

    const handleWordClick = () => {
        console.log(this.partOfSentence)
        if (dispatchType === PART_OF_SENTENCE.PREFIX) {
            console.log('adding prefix')
            dispatch(addPrefix({ text, color, partOfSentence: dispatchType }))
        }
        else {
            console.log('adding suffix')
            dispatch(addSuffix({ text, color, partOfSentence: dispatchType }))
        }
    }
    return (
