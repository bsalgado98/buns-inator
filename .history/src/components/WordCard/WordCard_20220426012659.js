...
import { addPrefix, addSuffix, removePrefix } from "../../slices/wordModifiersSlice"
import { PART_OF_SENTENCE } from "../../utility/constants"

const WordCard = ({ text = "N/A", color = "#000000", partOfSentence, applied=false }) => {
    const styles = {
        container: {
            style: {
                background: `${color}44`,
    ...
    const handleWordClick = () => {
        if (partOfSentence === PART_OF_SENTENCE.PREFIX) {
            if (applied) {
                dispatch(removePrefix({ text, color, partOfSentence }))
            }
            else {
